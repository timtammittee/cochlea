...
int SpikeGenerator(double *synout, double tdres, double t_rd_rest, double t_rd_init, double tau, double t_rd_jump, int nSites, double tabs, double trel, double spont, int totalstim, int nrep,double total_mean_rate,long MaxArraySizeSpikes, double *sptime, double *trd_vector)
{
    ...
    /* Max random array Size: nSites elements for oneSiteRedock
     * initialization, nSites elements for
     * preRelease_initialGuessTimeBins initialization 1 element for
     * Tref initialization, MaxArraySizeSpikes elements for Tref in
     * the loop, MaxArraySizeEvents elements one time for redocking,
     * another time for rate intervals Also, for before zero elements,
     * Averageley add 2nSites events (redock-and unitRate) and add
